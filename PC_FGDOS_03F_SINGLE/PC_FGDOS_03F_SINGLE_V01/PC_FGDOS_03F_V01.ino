...
void setup() {
  ...
  Serial.begin(9600); // higher for faster usart transfer, be careful not to lose data when transferring too fast
  while(!Serial);
...
void loop() {
  ...
  //digitalWrite(NSTBY_1,LOW); // see if still works with standby sensor
  //digitalWrite(PASSIVE,HIGH); // see if still works with passive sensor
  temperature_1 = read_reg(SS1,x0_TEMP);
  recharge_count_1 = read_reg(SS1,x1_RECHARGE_COUNT);
  collect_freq(SS2,&sens_freq_1,&ref_freq_1);
