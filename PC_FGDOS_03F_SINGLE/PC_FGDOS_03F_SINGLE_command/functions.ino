  ...
  while(!have_sens_freq || !have_ref_freq){
    if(!have_sens_freq){
      ...
      *sens_freq = *sens_freq * WINDOW_FACTOR;
      if ((freq_reg[0] & 0x04) == 0x04){
        Serial.println("sens freq overflow");
      }
    }
    if(!have_ref_freq){
      // reference frequency
      ...
      *ref_freq = *ref_freq * WINDOW_FACTOR;
      if ((freq_reg[0] & 0x04) == 0x04){
        Serial.println("ref freq overflow");
      }
    }
...
void collect_data_SS1(){
  ...
  if ((recharge_count & 0x80) == 0x80){
    Serial.println("recharge ongoing...");
  }
...
void collect_range_print(byte sensor){
  collect_range(sensor, &target_freq, &threshold_freq);
  Serial.print("TARGET: "); Serial.print(target_freq); Serial.print(", THRESHOLD: "); Serial.println(threshold_freq);
}

void fgdos_init_simple(byte sensor){
  ...
  write_reg(sensor,xC_CHARGE_SENS,0x70 | SENS);
  ...
  Serial.println();
  flag_print = true; // reprint data header
}

void fgdos_init(byte sensor){
  Serial.print("SENSOR "); Serial.println(abs(sensor-6));
  // set the reference oscillator and window measurement amount of pulses settings
  // bits (8:4) for recharging (enable autorech, internal pump at VB, rech by pump, discharge) and (3:2) for window (bit counting from lsb to msb)
  // windows set to 11 (4096 pulses)(00=32768 ck pulses per window)
  // TDIV (bit 0) to 0 or 1 (also change divisor for target and threshold!)
  // turn on (MSB to 1) or off auto recharge and nchp (bit 5) to 0 to enable charge pump
  write_reg(sensor,xB_RECHARGE_WINDOW,0xCD);
  Serial.print("window_factor "); Serial.println(WINDOW_FACTOR);
  // manual recharge off and sensitivity to low
  // MSB to switch on or off manual recharge, 3 LSBs to set sensitivity (100 low, 001 high)
  ...
  Serial.print("target ("); Serial.print(TARGET_FREQ); Serial.print(") , "); Serial.println((unsigned long)(reg_target << 10)*WINDOW_FACTOR);
  byte reg_threshold = floor(THRESHOLD_FREQ/WINDOW_FACTOR/1024);
  write_reg(sensor,xA_THRESHOLD,reg_threshold);
  Serial.print("threshold ("); Serial.print(THRESHOLD_FREQ); Serial.print(") , "); Serial.println((unsigned long)(reg_threshold << 10)*WINDOW_FACTOR);
...
void discharge_enable(byte sensor){
  // set endch to 1 and evbch to 0 on top of regular settings
  write_reg(sensor,0x0B,0x9D);
  // charge pump to max, epwr to 1 and ech to 1 to start (and 0 to stop)
  write_reg(sensor,0x0D,0x67);
  // PINS!
}

void discharge_disable(byte sensor){
  write_reg(sensor,0x0B,0xCD);
  write_reg(sensor,0x0D,0x47);
  // PINS!
}

void recharge_enable(byte sensor){
  // charge pump to max, epwr to 1 and ech to 1
  write_reg(sensor,0x0D,0x67);
