...
void collect_freq(byte sensor,long unsigned int *sens_freq, long unsigned int *ref_freq){
  ...
  // check frequencies even when recharge is ongoing
...
void fgdos_init(byte sensor){
  ...
  // TDIV (bit 0) to 1 for added precision
  // EAWR bit 7 to 0 to disable auto recharge
  write_reg(sensor,xB_RECHARGE_WINDOW,0x4D); // normal mode 0xCD
  ...
  // disconnect recharging system before setting targets, set pump to max (for shortest recharge time (bit 3:0))
  ...
  // also enable recharging again, set EPWR also to 1 for manual recharge
  // set internal pump (3 LSBs) to max output voltage
  write_reg(sensor,xD_RECHARGE_REF,0x47);
  Serial.println();
}

void activate_recharge(byte sensor){
  /*
   * MANUAL RECHARGE:
   * - ECH to 1: recharge allowed, is already set to 1 in setup
   * - EPWR to 1: is arleady set to 1 in setup
   * - FCH to 1 or 0: start or stop charging
   */
   write_reg(sensor,xC_CHARGE_SENS,0xF0 | SENS);  
}

void deactivate_recharge(byte sensor){
  /*
   * MANUAL RECHARGE:
   * - ECH to 1: recharge allowed, is already set to 1 in setup
   * - EPWR to 1: is arleady set to 1 in setup
